...
1. **Run the deterministic vision pipeline from the repository root:**
   ...
   ```
   Options: `--recursive`, `--limit N`, `--segment-ics`, `--max-side 0`, `--jpeg-quality 96`. Use `--segment-ics` when ICs in one image have mixed orientations or upside-down markings.
